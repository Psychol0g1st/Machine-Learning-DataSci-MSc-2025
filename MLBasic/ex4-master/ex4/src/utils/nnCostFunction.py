...
# Setup some useful variables
    ...
    X = np.c_[np.ones(m),X]
    in1 = X.dot(Theta1.T)  #5000x25
    out1 = np.c_[np.ones(m),sigmoid(in1)]
    in2 = out1.dot(Theta2.T)
    hyp = sigmoid(in2)
    ...
    for i in range(m):
        ...
        delta_2 = Theta2.T.dot(delta_3) * sigmoidGradient(np.r_[1,in1[i,:]])

        Theta1_grad += np.outer(delta_2[1:],X[i,:])
        Theta2_grad += np.outer(delta_3,out1[i,:])
