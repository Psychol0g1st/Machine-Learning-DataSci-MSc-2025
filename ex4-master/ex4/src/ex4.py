...
from .utils.sigmoidGradient import sigmoidGradient
from .utils.randInitializeWeights import randInitializeWeights
from .utils.checkNNGradients import checkNNGradients
...
def ex4():
    ...
    file_name = path.join(getcwd(), 'ex4', 'src', 'data', 'ex4data1')
    ...
    file_name = path.join(getcwd(), 'ex4', 'src', 'data', 'ex4weights')
    data = scipy.io.loadmat(file_name)
    Theta1 = data['Theta1']
    Theta2 = data['Theta2']
    y = np.squeeze(y)
    ...
    code you've written in nnCostFunction.py to return the partial derivatives
    of the parameters.
    """
    ...
    print('Cost at (fixed) debugging parameters (w/ lambda = 3): %f (this value should be about 0.575952)\n\n' % debug_J)
    ...
    You will now implement the "predict" function to use the neural network to predict
